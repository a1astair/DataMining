...
Unfortunately we had one of our group members, Nishant, drop the class. He was part of data preparation, which we already had two people working on, so we managed to split up the workload to keep on schedule.
...
Next, we will come up with more models that are more accurate in predicting the outcome of a match. If we have extra time, we will also come up with a model that takes into account the threat of relegation. For these other models we will split the work load between Alastair and Kolby, who will work on one model, and Graeme and Cole, who will work on another.
